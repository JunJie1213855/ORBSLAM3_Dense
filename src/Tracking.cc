...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    if(mSensor==System::STEREO || mSensor==System::IMU_STEREO){
        ...
        mTlr = settings->Tlr();
        mbIsDualCamera = (settings->cameraType() == Settings::KannalaBrandt);
    ...
    if(mSensor==System::STEREO || mSensor==System::IMU_STEREO)
    {
        ...
        // Dense: Q matrix already computed by Settings::stereoRectify
        Q = settings->Q();
    }
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::STEREO && !mbIsDualCamera)
        mCurrentFrame = Frame(mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    else if(mSensor == System::STEREO && mbIsDualCamera)
        mCurrentFrame = Frame(mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr);
    else if(mSensor == System::IMU_STEREO && !mbIsDualCamera)
