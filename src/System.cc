...
<<<<<<< HEAD
    ...
    // 核心三线程 tracking、local_ba、loop_closure，可视化两个线程 Viewer、 PointCloudMapping
...
>>>>>>> origin/main
            ...
            else
            {
                ...
                {
<<<<<<< HEAD
                    std::cerr << "Error, without Stereo.Type set" << "\n";
...
<<<<<<< HEAD
                case 2:
                    ...
                    {
                        cv::FileNode model_node = fsSettings["Stereo.TensorRTModelPath"];
                        cv::FileNode w_node = fsSettings["Stereo.InputWidth"];
                        cv::FileNode h_node = fsSettings["Stereo.InputHeight"];
                        if (!model_node.empty() && !w_node.empty() && !h_node.empty())
                        {
                            model_path = (std::string)model_node;
                            input_size = cv::Size(static_cast<int>(w_node), static_cast<int>(h_node));
                        }
                        else
                        {
                            std::cerr << "Error: IGEV requires TensorRTModelPath/InputWidth/InputHeight" << std::endl;
                            throw std::runtime_error("Missing TensorRT parameters");
                        }
                    }
                    break;
                case 3:
...
<<<<<<< HEAD
                // 创建 mapping 线程
                mpPointCloudMapping = std::make_shared<PointCloudMapping>(resolution, meank, stdthresh, unit, float(node_mindisp), float(node_maxdisp), real_type, input_size, model_path);
            }
        }
    ...
    Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point> &vImuMeas, string filename)
    {
        ...
        // std::cout << "start GrabImageStereo" << "\n";
        Sophus::SE3f Tcw = mpTracker->GrabImageStereo(imLeftToFeed, imRightToFeed, timestamp, filename);

        // std::cout << "out grabber" << "\n";
...
<<<<<<< HEAD
    ...
    void System::SaveTrajectoryEuRoC(const string &filename)
    {
        ...
        for (Map *pMap : vpMaps)
        {
            std::cout << "  Map " << std::to_string(pMap->GetId()) << " has " << std::to_string(pMap->GetAllKeyFrames().size()) << " KFs" << "\n";
            if (pMap->GetAllKeyFrames().size() > numMaxKFs)
            {
                numMaxKFs = pMap->GetAllKeyFrames().size();
                pBiggerMap = pMap;
