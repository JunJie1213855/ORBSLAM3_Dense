...
namespace ORB_SLAM3
{
                                                                                         ...
                                                                                         mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
    {
        ...
        if (mSensor == IMU_STEREO || mSensor == STEREO || mSensor == IMU_RGBD || mSensor == RGBD)
        {
            ...
            if (node_maxdisp.empty() || node_maxdisp.empty() || model_path.empty())
            {
            ...
            else
            {
                ...
                // 创建
                if (model_path.empty()) // 采用传统方法 ELAS = 0, SGBM = 1
                {

                    mpPointCloudMapping = std::make_shared<PointCloudMapping>(resolution, meank, stdthresh, unit, float(node_mindisp), float(node_maxdisp), real_type, cv::Size(width, height));
                }
                else // 采用TensorRT加速, IGEV = 2, LiteAnyStereo = 3
                {
                    mpPointCloudMapping = std::make_shared<PointCloudMapping>(resolution, meank, stdthresh, unit, float(node_mindisp), float(node_maxdisp), real_type, cv::Size(width, height), model_path);
                }
            }
        }
